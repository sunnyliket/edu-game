...
function getMoveVector() {
  ...
  return { ...move, active: length > 0.01 };
}
...
function draw3DScene() {
  ...
  add3DPlayer(faces, sprites, view);
...
function create3DView() {
  ...
  const yaw = player.facing + camera.lookX;
  const forward = { x: Math.cos(yaw), z: Math.sin(yaw) };
  const right = { x: -forward.z, z: forward.x };
  const camDistance = 245;
  const camX = camera.x - forward.x * camDistance;
  const camZ = camera.y - forward.z * camDistance;
  const camHeight = 144;
  ...
  return {
    ...
    cx: width / 2,
    cy: height * 0.64,
    focal: Math.min(width, height) * 0.86,
    camX,
    camZ,
    camHeight,
    forward,
...
function add3DPlayer(faces, sprites, view) {
  ...
  addPlayerShadow3D(sprites, view, x, z, size);

  addOrientedCuboid3D(faces, view, x, z, 34 * size, 23 * size, 43 * size, 43 * size, yaw, preset.shirt, preset.shirtStroke);
  addOrientedCuboid3D(faces, view, x + forward.x * 12 * size, z + forward.z * 12 * size, 18 * size, 5 * size, 55 * size, 18 * size, yaw, preset.accent, preset.shirtStroke);
  addOrientedCuboid3D(faces, view, x, z, 12 * size, 12 * size, 86 * size, 8 * size, yaw, preset.skin, preset.skinStroke);

  addOrientedCuboid3D(faces, view, x, z, 33 * size, 29 * size, 94 * size, 30 * size, yaw, preset.skin, preset.skinStroke);
  if (preset.hairStyle !== "none") {
    addOrientedCuboid3D(faces, view, x - forward.x * 3 * size, z - forward.z * 3 * size, 35 * size, 31 * size, 119 * size, 9 * size, yaw, preset.hair, preset.hair);
    addOrientedCuboid3D(faces, view, x + forward.x * 10 * size, z + forward.z * 10 * size, 34 * size, 8 * size, 111 * size, 13 * size, yaw, preset.hair, preset.hair);
  }
  if (preset.hairStyle === "long") {
    const leftHair = offsetPoint(x, z, right, -19 * size, forward, -2 * size);
    const rightHair = offsetPoint(x, z, right, 19 * size, forward, -2 * size);
    addOrientedCuboid3D(faces, view, leftHair.x, leftHair.z, 8 * size, 15 * size, 85 * size, 37 * size, yaw, preset.hair, preset.hair);
    addOrientedCuboid3D(faces, view, rightHair.x, rightHair.z, 8 * size, 15 * size, 85 * size, 37 * size, yaw, preset.hair, preset.hair);
    addOrientedCuboid3D(faces, view, x - forward.x * 14 * size, z - forward.z * 14 * size, 31 * size, 10 * size, 86 * size, 32 * size, yaw, preset.hair, preset.hair);
  }

  const leftLeg = offsetPoint(x, z, right, -8 * size, forward, walk * 6 * size);
  const rightLeg = offsetPoint(x, z, right, 8 * size, forward, -walk * 6 * size);
  addOrientedCuboid3D(faces, view, leftLeg.x, leftLeg.z, 9 * size, 11 * size, 6 * size, 38 * size, yaw, preset.pants, "#172554");
  addOrientedCuboid3D(faces, view, rightLeg.x, rightLeg.z, 9 * size, 11 * size, 6 * size, 38 * size, yaw, preset.pants, "#172554");
  addOrientedCuboid3D(faces, view, leftLeg.x + forward.x * 3 * size, leftLeg.z + forward.z * 3 * size, 12 * size, 17 * size, 0, 7 * size, yaw, preset.shoes, "#172554");
  addOrientedCuboid3D(faces, view, rightLeg.x + forward.x * 3 * size, rightLeg.z + forward.z * 3 * size, 12 * size, 17 * size, 0, 7 * size, yaw, preset.shoes, "#172554");

  const leftArm = offsetPoint(x, z, right, -25 * size, forward, -walk * 4 * size);
  const rightArm = offsetPoint(x, z, right, 25 * size, forward, walk * 4 * size);
  const armColor = preset.arm || preset.skin;
  const handColor = preset.hand || preset.skin;
  addOrientedCuboid3D(faces, view, leftArm.x, leftArm.z, 9 * size, 10 * size, 49 * size, 28 * size, yaw, armColor, preset.shirtStroke);
  addOrientedCuboid3D(faces, view, rightArm.x, rightArm.z, 9 * size, 10 * size, 49 * size, 28 * size, yaw, armColor, preset.shirtStroke);
  addOrientedCuboid3D(faces, view, leftArm.x, leftArm.z, 8 * size, 9 * size, 34 * size, 12 * size, yaw, handColor, preset.skinStroke);
  addOrientedCuboid3D(faces, view, rightArm.x, rightArm.z, 8 * size, 9 * size, 34 * size, 12 * size, yaw, handColor, preset.skinStroke);

  addPlayerFaceDetails3D(sprites, view, x, z, right, forward, preset, size);
}

function addPlayerShadow3D(sprites, view, x, z, size = 1) {
  const p = project3D(view, x, z, 2);
  if (!p) return;
  sprites.push({
    depth: p.depth + 2,
    draw() {
      const sx = clamp(24 * size * p.scale, 10, 42);
      const sy = clamp(10 * size * p.scale, 4, 18);
...
function addPlayerFaceDetails3D(sprites, view, x, z, right, forward, preset, size = 1) {
  const faceCenter = offsetPoint(x, z, right, 0, forward, 15.4 * size);
  const leftEye = offsetPoint(faceCenter.x, faceCenter.z, right, -6 * size, forward, 0);
  const rightEye = offsetPoint(faceCenter.x, faceCenter.z, right, 6 * size, forward, 0);
  const mouth = offsetPoint(faceCenter.x, faceCenter.z, right, 0, forward, 0);
  const pLeftEye = project3D(view, leftEye.x, leftEye.z, 109 * size);
  const pRightEye = project3D(view, rightEye.x, rightEye.z, 109 * size);
  const pMouth = project3D(view, mouth.x, mouth.z, 101 * size);
...
canvas.addEventListener("pointermove", (event) => {
  ...
  camera.lookX = clamp(input.lookStartX - dx * 0.005, -0.85, 0.85);
