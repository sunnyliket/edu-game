...
function isTargetVisible(item) {
  if (item.id === "door") return state.solved >= 60;
  return true;
}
...
function create3DView() {
  ...
  const yaw = -Math.PI / 2 + camera.lookX;
  const forward = { x: Math.cos(yaw), z: Math.sin(yaw) };
  const right = { x: -forward.z, z: forward.x };
  const camX = WORLD.width / 2;
  const camZ = WORLD.height + 126;
  const camHeight = 144;
...
function drawWallDecor3D(view) {
  ...
  drawFace3D(view, [p3(493, 40, 72), p3(690, 40, 72), p3(690, 40, 116), p3(493, 40, 116)], state.solved >= 60 ? "#dcfce7" : "#eef2f7", "#bfccd9", 2);
  drawScreenText3D(view, state.solved >= 60 ? "탈출문 열림" : "잠긴 탈출문", 540, 39, 98, 15, state.solved >= 60 ? "#16723a" : "#58677c");
...
function add3DPlayer(faces, sprites, view) {
  ...
  addOrientedCuboid3D(faces, view, x, z, 34, 23, 43, 43, yaw, "#2563eb", "#1d4ed8");
  addOrientedCuboid3D(faces, view, x + forward.x * 12, z + forward.z * 12, 18, 5, 55, 18, yaw, "#60a5fa", "#1d4ed8");
  addOrientedCuboid3D(faces, view, x, z, 12, 12, 86, 8, yaw, "#f3b58f", "#d18b65");

  addOrientedCuboid3D(faces, view, x, z, 33, 29, 94, 30, yaw, "#f6c39b", "#d18b65");
  addOrientedCuboid3D(faces, view, x - forward.x * 3, z - forward.z * 3, 35, 31, 119, 9, yaw, "#111827", "#111827");
  addOrientedCuboid3D(faces, view, x + forward.x * 10, z + forward.z * 10, 34, 8, 111, 13, yaw, "#111827", "#111827");

  const leftLeg = offsetPoint(x, z, right, -8, forward, walk * 6);
  const rightLeg = offsetPoint(x, z, right, 8, forward, -walk * 6);
  addOrientedCuboid3D(faces, view, leftLeg.x, leftLeg.z, 9, 11, 6, 38, yaw, "#1e3a8a", "#172554");
  addOrientedCuboid3D(faces, view, rightLeg.x, rightLeg.z, 9, 11, 6, 38, yaw, "#1e3a8a", "#172554");
  addOrientedCuboid3D(faces, view, leftLeg.x + forward.x * 3, leftLeg.z + forward.z * 3, 12, 17, 0, 7, yaw, "#1d4ed8", "#172554");
  addOrientedCuboid3D(faces, view, rightLeg.x + forward.x * 3, rightLeg.z + forward.z * 3, 12, 17, 0, 7, yaw, "#1d4ed8", "#172554");

  const leftArm = offsetPoint(x, z, right, -25, forward, -walk * 4);
  const rightArm = offsetPoint(x, z, right, 25, forward, walk * 4);
  addOrientedCuboid3D(faces, view, leftArm.x, leftArm.z, 8, 9, 42, 37, yaw, "#f3b58f", "#d18b65");
  addOrientedCuboid3D(faces, view, rightArm.x, rightArm.z, 8, 9, 42, 37, yaw, "#f3b58f", "#d18b65");

  addPlayerFaceDetails3D(sprites, view, x, z, right, forward);
}
...
function addPlayerFaceDetails3D(sprites, view, x, z, right, forward) {
