...
function triggerQuiz() {
    if (!window.QUIZ_DATA || window.QUIZ_DATA.length === 0) {
        ...
        alert('퀴즈 데이터가 없으므로 보상을 바로 적용합니다.');
    ...
    const qIdx = Math.floor(Math.random() * window.QUIZ_DATA.length);
    const pickedQuiz = window.QUIZ_DATA[qIdx];
    ...
    if (state.quizMode === 'boost') {
        playerInfo.textContent = `구조물 퀴즈 보상: ${state.pendingQuizReward.name}`;
    } else {
        playerInfo.textContent = `${getDisplayName(state.activeCardRecipient)} 과학 퀴즈!`;
    }
