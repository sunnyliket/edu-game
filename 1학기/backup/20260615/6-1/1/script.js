...
const clueQuestion = document.getElementById("clueQuestion");
const clueAnswer = document.getElementById("clueAnswer");
const clueOk = document.getElementById("clueOk");
...
const objects = [
  obj("clue1", "첫 번째 단서 카드", 590, 646, 36, 32, "clue", { stage: 1, solid: false }),
  obj("pocket", "자기 주머니", 590, 704, 32, 32, "pocket", { solid: false }),
  ...
  obj("earth", "지구모형", 906, 613, 92, 92, "station", { unitId: "earth", lockedByClue: true }),
...
function startGame() {
  ...
  showToast("단서 카드는 캐릭터 바로 앞에 있습니다.");
...
function update(dt) {
  ...
  player.vx = player.vx * friction + move.x * accel * dt;
  player.vy = player.vy * friction + move.y * accel * dt;
  ...
  camera.targetX = player.x + camera.lookX;
  camera.targetY = player.y + camera.lookY;
  camera.x += (camera.targetX - camera.x) * Math.min(1, dt * 5.8);
  camera.y += (camera.targetY - camera.y) * Math.min(1, dt * 5.8);
...
function isTargetVisible(item) {
  if (item.id === "clue1") return state.clueStage === 0;
  if (item.id === "pocket") return state.clueStage === 1;
  if (item.id === "door") return state.solved >= 60 && state.clueStage >= 2;
  return true;
}
...
function renderPrompt() {
  ...
  if (target.kind === "station") {
    ...
    if (target.lockedByClue && state.clueStage < 2) {
      promptText.textContent = `${target.name}: 먼저 두 번째 단서를 찾아야 합니다.`;
    } else if (solved >= unit.questions.length) {
...
function interact() {
  ...
  if (target.id === "clue1") {
    state.clueStage = Math.max(state.clueStage, 1);
    showClue(1, "단서 1", "등잔 밑이 어둡다.", "정답은 자기 주머니에 있었다.");
    return;
  }
  if (target.id === "pocket") {
    state.clueStage = Math.max(state.clueStage, 2);
    showClue(2, "단서 2", "동그라미 모형 있는 물건.", "정답은 지구모형.");
    return;
  }
  if (target.kind === "station") {
    if (target.lockedByClue && state.clueStage < 2) {
      showToast("아직 지구모형을 찾으라는 단서가 필요합니다.");
      return;
    }
  ...
  if (target.id === "door") {
    ...
    burstStars(window.innerWidth / 2, window.innerHeight * 0.38, 12);
    return;
  }
  showToast(`${target.name}에는 탈출에 필요한 표시가 없습니다.`);
}

function showClue(index, title, question, answer) {
  clueTitle.textContent = title;
  clueQuestion.textContent = `문제: ${question}`;
  clueAnswer.textContent = answer;
  openDialog(clueModal);
  showToast(`${title}를 찾았습니다.`);
  updateHud();
}
...
function checkMissionComplete() {
  if (state.solved >= 60 && state.clueStage >= 2) {
    showToast("모든 문제를 해결했습니다. 북쪽 탈출문으로 이동하세요.");
    burstStars(window.innerWidth / 2, 120, 10);
  }
}

function updateHud() {
  scoreText.textContent = String(state.score);
  clueText.textContent = `${Math.min(state.clueStage, 2)}/2`;
  progressText.textContent = `${state.solved}/60`;
  if (state.clueStage === 0) {
    missionText.textContent = "바로 앞의 단서 카드를 확인하세요.";
  } else if (state.clueStage === 1) {
    missionText.textContent = "등잔 밑이 어둡다. 캐릭터의 자기 주머니를 조사하세요.";
  } else if (state.solved < 60) {
    missionText.textContent = "지구모형 단서까지 찾았습니다. 과학실 물건을 조사해 4개 단원 문제를 모두 푸세요.";
  } else {
    missionText.textContent = "모든 문제를 해결했습니다. 북쪽 탈출문을 열고 나가세요.";
  }
  renderUnitProgress();
}
...
function drawWallDetails() {
  ...
  roundRect(484, 42, 212, 28, 6, state.solved >= 60 ? "#dcfce7" : "#eef2f7", "#bfccd9", 2);
  drawText(state.solved >= 60 ? "탈출문 열림" : "잠긴 탈출문", 536, 62, 15, state.solved >= 60 ? "#16723a" : "#58677c", "700");
...
function drawClues() {
  if (state.clueStage === 0) {
    const clue = objects.find((item) => item.id === "clue1");
    ctx.save();
    ctx.translate(clue.x, clue.y + Math.sin(performance.now() / 260) * 4);
    roundRect(-22, -18, 44, 36, 5, "#fff7ed", "#f59e0b", 3);
    drawText("단서", -14, 5, 13, "#9a3412", "900");
    ctx.restore();
  }
  if (state.clueStage === 1) {
    ctx.save();
    ctx.translate(player.x + 26, player.y - 20 + Math.sin(performance.now() / 260) * 3);
    roundRect(-26, -18, 52, 34, 5, "#ecfdf5", "#22c55e", 3);
    drawText("주머니", -20, 4, 12, "#166534", "900");
    ctx.restore();
  }
}
...
canvas.addEventListener("pointermove", (event) => {
  ...
  camera.lookX = clamp(input.lookStartX - dx * 0.45, -130, 130);
  camera.lookY = clamp(input.lookStartY - dy * 0.45, -95, 95);
});
...
closeClue.addEventListener("click", () => closeDialog(clueModal));
clueOk.addEventListener("click", () => closeDialog(clueModal));
