...
class Game {
    ...
    render(now) {
        ...
        // Draw Teacher
        {
            ...
            const teacherColor = this.teacherStunned ? '#666' : '#ff5252';

            // Glow
            ctx.save();
            const grad = ctx.createRadialGradient(tx, ty, 0, tx, ty, cs * 0.5);
            grad.addColorStop(0, this.teacherStunned ? 'rgba(100,100,100,0.25)' : 'rgba(255, 82, 82, 0.3)');
            grad.addColorStop(1, 'transparent');
            ctx.fillStyle = grad;
            ctx.beginPath();
            ctx.arc(tx, ty, cs * 0.5, 0, Math.PI * 2);
            ctx.fill();

            // Body
            ctx.globalAlpha = this.teacherStunned ? 0.5 : 1;
            ctx.font = `${cs * 0.55}px serif`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.shadowColor = teacherColor;
            ctx.shadowBlur = 12;
            ctx.fillText(this.teacherStunned ? '😵' : '👨‍🏫', tx, ty);
            ctx.restore();
        }
        ...
        // Draw Player
        {
            ...
            // Glow
            ctx.save();
            const grad = ctx.createRadialGradient(px, py, 0, px, py, cs * 0.5);
