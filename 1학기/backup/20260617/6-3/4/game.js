...
const spaceLevels = {
  ...
  medium: {
    // 2-story labyrinth with dead ends
    walls: [
      { x:260, y:0,   w:28, h:80  }, // top block
      { x:260, y:160, w:28, h:300 },
      { x:260, y:160, w:240, h:28 }, // horizontal floor
  ...
  hard: {
    // Complex maze with multiple dead ends & narrow corridors
    walls: [
      { x:200, y:0,   w:28, h:360 },
      { x:200, y:0,   w:180, h:30 }, // ceiling L
      { x:380, y:80,  w:28, h:380 },
      { x:460, y:0,   w:28, h:360 },
      { x:460, y:210, w:130, h:28 }, // mid floor
      { x:590, y:120, w:28, h:340 },
      { x:720, y:0,   w:28, h:310 },
      { x:720, y:400, w:150, h:28 }, // bottom shelf
      { x:870, y:80,  w:28, h:380 },
      { x:1010,y:150, w:28, h:310 },
    ],
    // Tiny 30px platforms – precision required
    platforms: [
      { x:1200, y:360, w:30,  h:18, s:'space' },
      { x:1290, y:280, w:30,  h:18, s:'space' },
      { x:1380, y:200, w:30,  h:18, s:'space' },
      { x:1470, y:290, w:30,  h:18, s:'space' },
      { x:1560, y:370, w:30,  h:18, s:'space' },
      { x:1650, y:280, w:30,  h:18, s:'space' },
      { x:1740, y:190, w:30,  h:18, s:'space' },
      { x:1830, y:280, w:30,  h:18, s:'space' },
      { x:1920, y:370, w:30,  h:18, s:'space' },
      { x:2010, y:280, w:30,  h:18, s:'space' },
      { x:2100, y:190, w:30,  h:18, s:'space' },
      { x:2200, y:300, w:120, h:18, s:'space' }, // final wide platform
    ],
    hazards: [
...
function resetGame() {
  ...
  const p = gs.player;
  p.x=150; p.y=GROUND_Y-40; p.vx=0; p.vy=0; p.grounded=false; p.spinAngle=0;
  p.flying = false;
  gs.camX = 0;
  gs.particles = [];

  boss.active=true; boss.projectile=null;
  boss.cinematic=false; boss.cinTime=0; boss.crownY=0; boss.portalOpen=false;
...
function loadQuestion() {
  ...
  const criteria = isBoss ? '20개 모두 맞히기' : '12개 이상 맞히기';
...
function updatePhysics() {
  ...
  // Movement
  if (gs.keys['a'] || gs.keys['A'] || gs.keys['ArrowLeft']) {
...
function triggerGameOver(reason) {
  playSound('gameover');
  document.getElementById('gameover-reason').textContent = reason + ' 조금 더 공부하십시오. 화이팅!';
...
function drawSpaceHubPads() {
  ...
  pads.forEach(pad => {
    ...
    // Interact prompt
    if (Math.abs((p.x + p.w/2) - pad.x) < 65) {
      ctx.shadowBlur = 0;
...
function drawPlayer() {
  ...
  if (boss.cinematic) {
    ...
    let sx = 1, sy = 1;
    if (jumping) { sx = 0.9; sy = 1.1; }
    else if (Math.abs(p.vx) > 0.5) { sx = 1.05; sy = 0.95; }
    ctx.translate(p.x + p.w/2, p.y + p.h/2);
    ctx.scale(sx, sy);
  }
  ...
  if (flying) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.ellipse(-25, -6 + flap * 0.12, 15, 8 + Math.abs(flap) * 0.25, -0.45, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
    ctx.beginPath();
    ctx.ellipse(25, -6 - flap * 0.12, 15, 8 + Math.abs(flap) * 0.25, 0.45, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  ...
  ctx.beginPath(); ctx.arc(0, -14+bY, 3, 0, Math.PI); ctx.stroke();
