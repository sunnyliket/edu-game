...
function ox(category, question, isTrue) {
    return {
        category,
        question,
        options: ['O', 'X'],
...
const QUESTIONS = [
    ...
    // 산과 염기 OX
    ox('산과 염기', '식초나 레몬즙은 산성 물질이다.', true),
    ox('산과 염기', '빨래할 때 쓰는 비눗물이나 유리 세정제는 염기성 물질이다.', true),
    ox('산과 염기', '산성 용액에 푸른색 리트머스 종이를 넣으면 붉은색으로 변한다.', true),
    ox('산과 염기', '염기성 용액에 페놀프탈레인 용액을 떨어뜨리면 아무런 색깔 변화가 없다.', false),
    ox('산과 염기', '붉은 양배추를 끓여서 만든 붉은 양배추 시험지는 천연 지시약이다.', true),
    ox('산과 염기', '산성 용액인 묽은 염산에 달걀 껍데기를 넣으면 기체가 발생하면서 달걀 껍데기가 녹는다.', true),
    ox('산과 염기', '염기성 용액은 대리석을 녹이는 성질이 있다.', false),
    ox('산과 염기', '붉은 양배추 지시약은 산성 용액에 넣으면 푸른색이나 노란색으로 변한다.', false),
    ox('산과 염기', '산성 용액에 염기성 용액을 조금씩 섞으면 산성의 성질이 점점 약해진다.', true),
    ox('산과 염기', '생선을 먹을 때 레몬즙을 뿌리는 것은 생선의 비린내를 산성 물질로 줄이기 위해서이다.', true),
    ox('산과 염기', '산성 용액과 염기성 용액은 눈으로 보아도 색깔이 뚜렷하게 달라서 쉽게 구별할 수 있다.', false),
    ox('산과 염기', '모든 산성 물질과 염기성 물질은 안전하므로 맛을 보거나 손으로 찍어 만져도 된다.', false),
    ox('산과 염기', '염기성 용액에 삶은 달걀 흰자를 넣으면 달걀 흰자가 흐물흐물하게 녹는다.', true),
    ox('산과 염기', '논이나 밭의 땅이 산성으로 변했을 때 이를 해결하기 위해 석회 가루를 뿌리기도 한다.', true),
    ox('산과 염기', '지시약이란 용액에 넣었을 때 산성인지 염기성인지 색깔 변화로 알려주는 물질이다.', true),

    // 물체의 운동 OX
    ox('물체의 운동', '과학에서 물체의 운동을 나타낼 때는 물체가 이동하는 데 걸린 시간과 이동 거리를 함께 나타내어야 한다.', true),
    ox('물체의 운동', '걸어가는 사람과 달리는 자동차가 같은 시간 동안 이동했다면, 이동 거리가 더 짧은 자동차가 더 빠르다.', false),
    ox('물체의 운동', '100 m 달리기 시합처럼 이동 거리가 같을 때는 걸린 시간이 더 짧은 사람이 더 빠르다.', true),
    ox('물체의 운동', '물체의 빠르기를 나타내는 속력은 걸린 시간 나누기 이동 거리로 계산한다.', false),
    ox('물체의 운동', '어떤 자전거가 2초 동안 10 m를 이동했다면, 이 자전거의 속력은 5 m/s이다.', true),
    ox('물체의 운동', '속력의 단위 중 하나인 km/h는 킬로미터 퍼 아워라고 읽으며, 1시간 동안 이동한 킬로미터 수를 뜻한다.', true),
    ox('물체의 운동', '물체의 위치를 나타낼 때는 기준점, 방향, 거리 세 가지 요소가 모두 필요하다.', true),
    ox('물체의 운동', '물체의 운동을 그래프로 나타낼 때, 가로축에는 주로 이동 거리를 쓴다.', false),
    ox('물체의 운동', '일정한 속력으로 움직이는 물체는 시간이 지남에 따라 이동 거리가 일정하게 늘어난다.', true),
    ox('물체의 운동', '초속 10 m/s로 달리는 스케이트보드는 시속 5 km/h로 걷는 사람보다 느리다.', false),
    ox('물체의 운동', '과속 방지턱이나 카메라가 있는 도로 표지판은 물체의 속력과 관련된 안전장치이다.', true),
    ox('물체의 운동', '일정한 시간 동안 이동한 거리를 비교할 때, 이동 거리가 짧을수록 속력이 빠르다고 한다.', false),
    ox('물체의 운동', '교통사고를 예방하기 위해 생활 속에서 물체의 속력을 제한하는 구역이 존재한다.', true),
    ox('물체의 운동', '물체가 이동할 때 방향이 바뀌더라도 속력만 일정하면 항상 같은 위치에 도달하게 된다.', false),
    ox('물체의 운동', '달리기 시합을 할 때 출발선과 도착선이 정해져 있다면, 이동 거리를 일정하게 맞춘 것이다.', true),

    // 식물의 구조와 기능 OX
    ox('식물의 구조와 기능', '식물을 땅에 단단히 고정해 주고, 흙 속의 물과 양분을 흡수하는 부분은 뿌리이다.', true),
    ox('식물의 구조와 기능', '식물의 물관은 잎에서 만든 양분이 이동하는 길이고, 체관은 물이 이동하는 길이다.', false),
    ox('식물의 구조와 기능', '식물의 잎은 주로 햇빛을 받아 식물에게 필요한 양분을 만드는 일을 한다.', true),
    ox('식물의 구조와 기능', '식물이 양분을 만드는 과정인 광합성은 햇빛이 없는 어두운 밤에도 잘 일어난다.', false),
    ox('식물의 구조와 기능', '식물이 광합성을 할 때 공기 중에서 흡수하는 기체는 이산화 탄소이다.', true),
    ox('식물의 구조와 기능', '식물이 광합성을 하고 나면 사람과 동물이 숨을 쉴 때 필요한 산소가 만들어져 밖으로 나온다.', true),
    ox('식물의 구조와 기능', '식물 몸속에 있던 물이 잎을 통해 수증기 상태로 공기 중으로 나가는 현상을 증산 작용이라고 한다.', true),
    ox('식물의 구조와 기능', '잎에서 광합성으로 처음 만들어진 양분은 낮 동안 주로 녹말의 형태로 바뀌어 잎에 저장된다.', true),
    ox('식물의 구조와 기능', '식물의 줄기는 뿌리에서 흡수한 물이 잎이나 열매로 갈 수 있도록 이동 통로 역할을 한다.', true),
    ox('식물의 구조와 기능', '꽃의 중심에 있는 암술은 나중에 자라서 우리가 먹는 과일과 같은 열매가 된다.', true),
    ox('식물의 구조와 기능', '수술에서 만들어진 꽃가루가 암술머리에 붙는 현상을 수분 또는 꽃가루받이라고 한다.', true),
    ox('식물의 구조와 기능', '꽃잎은 주로 초록색이며 꽃의 가장 아래쪽에서 꽃을 받쳐 주고 보호하는 역할을 한다.', false),
    ox('식물의 구조와 기능', '사막에 사는 선인장의 뾰족한 가시는 사실 잎이 변한 것이다.', true),
    ox('식물의 구조와 기능', '잎에 도달한 물이 수증기가 되어 기공을 통해 밖으로 빠져나가는 것을 증산 작용이라고 한다.', true),
    ox('식물의 구조와 기능', '꽃은 대부분 암술, 수술, 꽃잎, 꽃받침으로 이루어져 있다.', true),

    // 지구의 운동 OX
    ox('지구의 운동', '지구는 자전축을 중심으로 하루에 한 바퀴씩 회전하며, 이를 지구의 자전이라고 한다.', true),
    ox('지구의 운동', '지구의 자전 방향은 동쪽에서 서쪽이다.', false),
    ox('지구의 운동', '하루 동안 태양과 달이 동쪽에서 서쪽으로 움직이는 것처럼 보이는 것은 지구의 자전 때문이다.', true),
    ox('지구의 운동', '지구가 태양을 중심으로 1년에 한 바퀴씩 도는 것을 지구의 공전이라고 한다.', true),
    ox('지구의 운동', '지구가 공전하기 때문에 계절에 따라 밤하늘에 보이는 별자리가 달라진다.', true),
    ox('지구의 운동', '겨울철 별자리를 여름철 밤 9시에 똑같이 관찰할 수 있다.', false),
    ox('지구의 운동', '여러 날 동안 같은 시각에 달을 관찰하면 달의 위치는 서쪽에서 동쪽으로 조금씩 이동한다.', true),
    ox('지구의 운동', '태양이 뜨고 질 때까지의 시간을 낮, 태양이 지고 다시 떠오르기 시작할 때까지의 시간을 밤이라고 한다.', true),
    ox('지구의 운동', '어느 계절의 밤하늘에서 오랫동안 관찰할 수 있는 별자리를 그 계절의 대표적인 별자리라고 한다.', true),
    ox('지구의 운동', '여름의 대표적인 별자리는 사자자리이다.', false),
    ox('지구의 운동', '봄의 대표적인 별자리는 오리온자리이다.', false),
    ox('지구의 운동', '가을의 대표적인 별자리는 페가수스자리이다.', true),
    ox('지구의 운동', '겨울철 대표 별자리에는 쌍둥이자리, 큰개자리, 물고기자리가 있다.', false),
    ox('지구의 운동', '여름에도 새벽이나 관찰 시각에 따라 겨울철 별자리를 볼 수 있다.', true),
    ox('지구의 운동', '낮과 밤이 생기는 근본적인 원인은 지구의 자전 때문이다.', true),
];
