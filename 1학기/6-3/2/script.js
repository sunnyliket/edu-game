...
function q(question, choices, answer, explain = "제공된 답안 기준으로 확인한 문제입니다.") {
  return { question, choices, answer, explain };
}

const expandedAcidBaseQuestions = [
  q("유리 세정제나 비눗물을 손으로 만졌을 때 2개 모두 공통으로 느껴지는 촉감은 무엇일까요?", ["미끈거린다.", "아무런 느낌도 없다.", "끈적끈적하다."], 0, "염기성 용액은 미끈거리는 느낌이 날 수 있습니다."),
  q("다음 중 레몬즙이나 식초를 맛보았을 때 2개 모두 공통으로 느껴지는 맛은 무엇일까요?", ["짠맛", "쓴맛", "신맛"], 2, "레몬즙과 식초는 산성 용액이라 신맛이 납니다."),
  q("다음 중 산성 용액에 리트머스 종이를 넣었을 때 나타나는 색깔 변화로 올바른 것은 무엇일까요?", ["푸른색 리트머스 종이가 붉은색으로 변한다.", "붉은색 리트머스 종이가 푸른색으로 변한다.", "푸른색 리트머스 종이가 푸른색으로 변한다."], 0, "산성 용액은 푸른색 리트머스 종이를 붉은색으로 바꿉니다."),
  q("붉은색 리트머스 종이에 비눗물을 떨어뜨렸을 때 나타나는 색깔 변화로 옳은 것은 무엇일까요?", ["노란색으로 변한다.", "푸른색으로 변한다.", "붉은색 그대로 있다."], 1, "비눗물은 염기성이므로 붉은색 리트머스 종이를 푸른색으로 바꿉니다."),
  q("페놀프탈레인 용액은 염기성 용액을 만났을 때만 색깔이 변합니다. 어떤 색으로 변할까요?", ["붉은색", "노란색", "초록색"], 0, "페놀프탈레인 용액은 염기성에서 붉은색 계열로 변합니다."),
  q("산성 용액에 염기성 용액을 조금씩 넣을 때 일어나는 변화로 옳은 설명은 무엇일까요?", ["용액의 색깔이 투명해집니다.", "용액의 산성이 점점 강해집니다.", "산성이 점점 약해지다가 염기성 용액으로 변합니다."], 2, "염기성 용액을 넣으면 산성이 약해지다가 염기성이 될 수 있습니다."),
  q("생활 속에서 산성 용액과 염기성 용액을 이용하는 사례 중 성질이 다른 하나는 무엇일까요?", ["변기 세정제를 사용하여 때를 닦아냅니다.", "생선 비린내를 없애기 위해 레몬즙을 뿌립니다.", "속이 쓰릴 때 제산제를 먹습니다."], 0),
  q("양배추 지시약을 산성 용액에 넣었을 때 나타나는 색은 무엇일까요?", ["붉은색 계열", "푸른색이나 초록색", "노란색"], 0, "양배추 지시약은 산성에서 붉은색 계열로 변합니다."),
  q("다음 중 염기성 용액에 해당하는 물질을 모두 고르세요.", ["오렌지 주스, 토마토즙", "식초, 사이다", "유리 세정제, 빨래 비눗물"], 2, "유리 세정제와 빨래 비눗물은 염기성 용액의 예입니다."),
  q("식초는 산성이다.", ["O", "X"], 1),
  q("염기성 용액은 대리석을 녹이는 성질이 있다.", ["O", "X"], 1),
  q("페놀프탈레인 용액은 산성 용액을 붉게 변하게 한다.", ["O", "X"], 0),
  q("염기성 용액에 삶은 달걀흰자를 넣으면 딱딱해진다.", ["O", "X"], 1),
  q("대리석으로 만든 조각상에 산성비가 내리면 대리석이 녹을 수 있다.", ["X", "O"], 1, "산성비가 대리석을 손상시킬 수 있습니다."),
  q("용액의 성질은 무엇으로 알 수 있나요?", ["지시약", "음료", "대리석"], 0, "지시약으로 용액의 성질을 알 수 있습니다.")
];

const expandedMotionQuestions = [
  q("물체가 실제로 움직인 경로의 총 길이를 무엇이라고 하나요?", ["이동 거리", "이동 시간", "걸린 시간", "걸린 거리"], 0, "물체가 실제로 움직인 경로의 총 길이는 이동 거리입니다."),
  q("치타는 거북이보다 속력이 빠를까요?", ["X", "O"], 1, "치타는 거북이보다 빠릅니다."),
  q("100m 달리기 시합을 했습니다. 다음 중 가장 빠른 사람은 누구인가요?", ["17초 걸린 지안", "15초 걸린 수아", "16초 걸린 연희", "13초 걸린 민우"], 3, "같은 거리에서는 시간이 가장 짧은 사람이 가장 빠릅니다."),
  q("속력을 구하는 방법은 무엇인가요?", ["걸린 시간 × 이동 거리", "이동 거리 ÷ 걸린 시간", "이동한 물체 × 이동 거리", "이동한 물체 ÷ 걸린 시간"], 1, "속력은 이동 거리를 걸린 시간으로 나누어 구합니다."),
  q("자전거를 타고 3시간 동안 36km를 이동했습니다. 이 자전거의 속력은 얼마인가요?", ["30km/h", "9km/h", "108km/h", "12km/h"], 3, "36km ÷ 3시간 = 12km/h입니다."),
  q("민우는 자전거를 타고 2시간 동안 24km를 달렸고, 윤서는 인라인스케이트를 타고 3시간 동안 30km를 달렸습니다. 두 사람의 빠르기를 올바르게 비교한 것은 무엇인가요?", ["민우의 속력이 더 빠르다.", "윤서의 속력이 더 빠르다.", "속력이 두 사람 모두 같다."], 0, "민우는 12km/h, 윤서는 10km/h입니다."),
  q("다음 중 속력이 가장 빠른 것은 무엇인가요?", ["15km/h", "13km/h", "16km/h"], 2, "16km/h가 가장 빠릅니다."),
  q("과학에서 말하는 물체의 운동에 대한 설명으로 가장 올바른 것은 무엇인가요?", ["물체의 모양이나 크기가 변하는 것이다.", "시간이 지남에 따른 위치의 변화를 운동이라 한다.", "물체가 제자리에 가만히 멈춰 서 있는 것이다."], 1, "운동은 시간이 지남에 따른 위치 변화입니다."),
  q("물체의 위치가 시간이 지남에 따라 변하는 것을 무엇이라고 하나요?", ["거리", "운동", "정지", "무게"], 1, "위치가 시간이 지남에 따라 변하는 것은 운동입니다."),
  q("물체의 빠르기를 나타내는 말로, 단위 시간 동안 이동한 거리를 무엇이라고 하나요?", ["속력", "무게", "거리", "시간"], 0, "단위 시간 동안 이동한 거리는 속력입니다."),
  q("여러 명의 학생이 100m 달리기를 했습니다. 이때 가장 빠른 사람은 누구일까요?", ["걸린 시간이 가장 긴 사람", "걸린 시간이 가장 짧은 사람"], 1, "같은 거리에서는 걸린 시간이 가장 짧은 사람이 빠릅니다."),
  q("다음 중 속력의 단위로 알맞지 않은 것은 무엇인가요?", ["cm/s", "km/h", "kg"], 2, "kg은 질량의 단위입니다."),
  q("과학에서 물체가 시간이 지남에 따라 위치가 변하는 것을 운동이라고 합니다.", ["O", "X"], 0),
  q("교통수단의 빠르기를 나타낼 때 주로 사용하는 단위로, 1시간 동안 이동한 킬로미터 수를 뜻하는 것은 km/h이라고 합니다.", ["O", "X"], 0),
  q("민호는 체육 시간에 50m를 달리는 데 10초가 걸렸습니다. 민호의 속력은 5m/s이다.", ["O", "X"], 0, "50m ÷ 10초 = 5m/s입니다.")
];

const expandedPlantQuestions = [
  q("식물을 이루고 있는 기본 단위를 광합성이라고 합니다.", ["O", "X"], 1),
  q("식물의 잎은 햇빛을 받아서 식물이 먹고살 양분을 스스로 만든다.", ["O", "X"], 0),
  q("뿌리는 물을 흡수한다.", ["O", "X"], 0),
  q("식물의 뿌리가 하는 기능 중, 땅속에 식물이 쓰러지지 않도록 단단히 붙잡아 주는 기능은 무엇인가요?", ["흡수 작용", "저장 작용", "지지 작용"], 2),
  q("뿌리에 양분을 저장하는 식물이 있다.", ["O", "X"], 0),
  q("식물의 뿌리는 땅속에 식물이 쓰러지지 않도록 단단히 붙잡아 주는 지지 작용을 합니다.", ["O", "X"], 0),
  q("식물을 분류할 때 꽃이 피는가는 올바른 분류 기준이 될 수 있습니다.", ["O", "X"], 0),
  q("생태계에서 햇빛과 물을 받아 스스로 양분을 만드는 식물은 소비자에 해당합니다.", ["O", "X"], 1),
  q("생태계 구성 요소 중 햇빛, 물, 토양, 공기 등은 비생물 요소라고 합니다.", ["O", "X"], 0)
];

const expandedEarthQuestions = [
  q("별자리는 움직인다.", ["O", "X"], 1),
  q("자전은 동에서 서로 자전한다.", ["O", "X"], 1),
  q("지구의 공전 주기는 얼마인가요?", ["2년", "1년", "6개월", "하루"], 1),
  q("태양 빛이 비치는 곳은 낮인가요?", ["O", "X"], 0),
  q("지구는 스스로 회전하나요?", ["O", "X"], 0),
  q("지구는 자전하면서 공전하나요?", ["O", "X"], 0),
  q("봄철 남쪽 하늘에 보이는 별자리는 무엇인가요?", ["백조자리", "페가수스자리", "쌍둥이자리", "목동자리"], 3)
];

const expandedQuizPools = {
  shop: { pool: expandedAcidBaseQuestions, count: 5 },
  salon: { pool: expandedMotionQuestions, count: 5 },
  final: {
    groups: [
      { pool: expandedPlantQuestions, count: 2 },
      { pool: expandedEarthQuestions, count: 1 },
      { pool: expandedAcidBaseQuestions, count: 1 },
      { pool: expandedMotionQuestions, count: 1 }
    ]
  }
};
...
function getStageQuestions(stageId) {
  const config = expandedQuizPools[stageId];
  if (!config) {
    return stages[stageId].questions;
  }

  if (config.groups) {
    return config.groups.flatMap((group) => sampleQuestions(group.pool, group.count));
  }

  return sampleQuestions(config.pool, config.count);
}
...
function triggerWrongEffect() {
  gameShell.classList.remove("wrong-answer-effect");
  void gameShell.offsetWidth;
  gameShell.classList.add("wrong-answer-effect");
  window.setTimeout(() => {
    gameShell.classList.remove("wrong-answer-effect");
  }, 620);
}
